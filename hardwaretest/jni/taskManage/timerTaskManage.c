...
static int changeParameter(struct TimerOps* base, int startTime, int loopTime,
		int loopCount);
...
TimerOps timerOps = { .start = start, .changeParameter = changeParameter,
		.changeArg = changeArg, .reset = reset, .stop = stop, };
static int start(struct TimerOps* base) {
	...
	if (timerServer->timerid == NULL) {
		...
		if (timer_create(CLOCK_MONOTONIC, &hard_evp, &(timerServer->timerid))
				== -1) {
...
pTimerOps createTimerTaskServer(int startTime, int loopTime, int loopCount,
		int startThread, pThreadFunc taskFunc, void *arg, int dataLen) {

	struct sigevent hard_evp;
	pTimerServer timerServer = (pTimerServer) malloc(
			sizeof(struct TimerServer));
	if (timerServer == NULL)
		goto fail0;
	...
	fail3: timer_delete(timerServer->timerid);
	fail2: free(timerServer->handleArg);
	timerServer->handleArg = NULL;
	fail1: free(timerServer);
	fail0: return NULL;
...
static void notify_function(union sigval signo) {
	...
	if (timerServer->handle != NULL) {
		...
		if (timerServer->startThread != 0) {
			pthread_create(&pid, NULL, timerServer->handle,
					timerServer->handleArg);
			pthread_detach(pid);
		} else {
