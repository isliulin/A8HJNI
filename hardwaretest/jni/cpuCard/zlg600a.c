...
static int _send_command(const unsigned char *cmd, size_t len)
{
	...
	if(ret == -2)	{ // timeout / uart read error
	...
	if(ctl->respons[respons_len - 2] != _get_bcc(ctl->respons, respons_len - 2)) {
...
static int ctl_serial_baud_sync(void)
{
	...
	for(i = 0; i < 3; i++) {
		...
		if((ret = ctl->uart->read(ctl->uart, ctl->respons, 
				MAX_BUF_SIZE, time_out_ms)) == 1)
			break;
	}
...
{
	const unsigned char cmd[] = {0x08, 0x06, 0x4D, 0x02, 0x00, 0x26, 0x98, 0x03};
	int ret = _send_command(cmd, sizeof(cmd)/sizeof(cmd[0]));
...
{
	const unsigned char cmd[] = {0x09, 0x06, 0x50, 0x03,
			0x00, 0x00, 0x00, 0xA3, 0x03};
	return _send_command(cmd, sizeof(cmd)/sizeof(cmd[0]));
}

int picc_B_active(unsigned char *info, unsigned char *len __unused)
{
	const unsigned char cmd[] = {0x08, 0x06, 0x4E, 0x02, 0x00, 0x00, 0xBD, 0x03};
	int ret = _send_command(cmd, sizeof(cmd)/sizeof(cmd[0]));
...
{
	const unsigned char cmd[] = {0x0A, 0x06, 0x53, 0x04,
			0x95, 0x22, 0x49, 0x38, 0x62, 0x03};
	return _send_command(cmd, sizeof(cmd)/sizeof(cmd[0]));
}
