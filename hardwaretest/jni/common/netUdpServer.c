...
static int _readAndWait(struct UdpOps* base, unsigned char * lpBuff,
...
static int _setsockopt(struct UdpOps* base, int level, int optname,
		const void *optval, socklen_t optlen);
static int setRemoteInfo(struct UdpOps* base, struct sockaddr_in remoteInfo);

static UdpOps ops = { .setHandle = setHandle, .read = _readAndWait, .ack = _ack,
		.write = _udpWrite, .joinMulticast = joinMulticast, .getRemoteInfo =
				getRemoteInfo, .setRemoteInfo = setRemoteInfo, .setsockopt =
				_setsockopt, };
	...
	while (udpServer->recvThreadId->check(udpServer->recvThreadId)) {
		...
		for (i = 0; i < nfds; ++i) {
			if (events[i].data.fd == udpServer->socketFd) {
				...
				readLen = recvfrom(udpServer->socketFd, readBuff,
						sizeof(readBuff), 0,
						(struct sockaddr*) &udpServer->remoteInfo,
						(socklen_t*) &sockaddrLen);
				//加入队列
				if (readLen > 0) {
					udpServer->bufferOps->push(udpServer->bufferOps, readBuff,
							readLen);
				} else {
	...
	while (udpServer->recvThreadId->check(udpServer->recvThreadId)) {
		...
		case TRI_DATA:
			do {
				...
				pullLen = udpServer->bufferOps->pull(udpServer->bufferOps,
						recvBuf, sizeof(recvBuf));
				if (pullLen > 0) {
					if (udpServer->recvFunc) {
						if (udpServer->parseFunc) {
							handleLen = udpServer->parseFunc(recvBuf, pullLen,
									validBuf, &validLen);
							if ((handleLen > 0)) {
								if (validLen > 0) {
									ret = udpServer->bufferOps->deleteLeft(
											udpServer->bufferOps, handleLen);
									if (ret < 0) {
...
static int setHandle(struct UdpOps* base, UDPRecvFunc recvFunc,
	...
	return true;
	fail2: free(udpServer->recvThreadId);
	fail1: free(udpServer->parseThreadId);
	fail0: return -1;
}
static int _ack(struct UdpOps* base, unsigned char * data, int size) {
	pUdpServer udpServer = (pUdpServer) base;
...
static int _udpWrite(struct UdpOps* base, const char * data, int size,
	...
	ret = sendto(udpServer->socketFd, data, size, 0,
...
static int udp_getSocketFd(int bandPort) {
	...
	if (setsockopt(udpSocketfd, SOL_SOCKET, SO_BROADCAST, &broadcastPerm,
...
static int joinMulticast(struct UdpOps* base, unsigned int multiaddr) {
	...
	if (setsockopt(udpServer->socketFd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq,
			sizeof(mreq)) < 0) {
	...
	int on = 1;
	//设置允许发生广播
	if (setsockopt(udpServer->socketFd, SOL_SOCKET, SO_BROADCAST, &on,
			sizeof(on)) < 0) {
		LOGE("as_socket setsockopt SO_BROADCAST:");
		return -1;
	}
	if (setsockopt(udpServer->socketFd, IPPROTO_IP, IP_MULTICAST_IF,
			&mreq.imr_multiaddr, sizeof(mreq.imr_multiaddr)) < 0) {
		LOGE("IP_MULTICAST_IF\n");
		//return -1;
	}
	int loop = 0;
	if (setsockopt(udpServer->socketFd, IPPROTO_IP, IP_MULTICAST_LOOP, &loop,
			sizeof(loop)) < 0) {
